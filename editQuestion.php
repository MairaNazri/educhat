...
$stmt = $conn->prepare("SELECT q.*, qz.title AS quiz_title FROM question q 
...
$quizID = $question['quizID']; // Store this for redirect
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($text) && in_array($type, ['multiple_choice', 'short_answer'])) {
        $stmt = $conn->prepare("UPDATE question SET question_text = ?, question_type = ? WHERE questionID = ?");
        $stmt->bind_param("ssi", $text, $type, $questionID);
...
<body>
  ...
  <div class="main" id="main">
    ...
    <div class="content">
      <div class="quiz-info">
        <h3>Editing question for: <?= htmlspecialchars($question['quiz_title']) ?> (Quiz ID: <?= $quizID ?>)</h3>
      </div>
